...
INSERT INTO backend.tariffs (station_id, price_per_kwh, gst_percent, platform_fee_percent, session_fee, currency)
SELECT station.id, seed.price_per_kwh, seed.gst_percent, seed.platform_fee_percent, seed.session_fee, seed.currency
FROM (
    VALUES
    ('Veltrak EV Hub - Anna Nagar', 20.00, 18.0, 12.0, 0.0, 'INR'),
    ('Veltrak EV Hub - OMR', 20.00, 18.0, 12.0, 0.0, 'INR')
) AS seed(station_name, price_per_kwh, gst_percent, platform_fee_percent, session_fee, currency)
JOIN backend.stations station ON station.name = seed.station_name
